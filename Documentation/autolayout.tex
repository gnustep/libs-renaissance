...
Apple OSX, so hardcoding the size and position of popup buttons
creates user interfaces which are not portable between the two
systems: a popup button which is perfectly sized on Apple OSX might be
...
button to zero.  Renaissance AutoLayout does support (and encourage the use
of) minimum sizes.

\end{enumerate}

In theory, given those two basic requirements, not much more can be
said: there are infinite ways of organizing the autolayout of a
...
In summary, in Renaissance there are four basic types of boxes: horizontal
standard boxes, horizontal proportional boxes, vertical standar boxes,
and vertical proportional boxes.
...
{
  [self setFrame: [self minimumSizeForContent]];
}
\end{verbatim}
...
This is one of the key concepts of the gnustep-gui Autolayout
framework - that some views (the ones which are not necessarily able
to display all information when they are at their minimum size)
benefit from being expanded, while other views (the ones which always
display all information when they are at their minimum size) do not
benefit from being expanded - actually they are better not expanded
since we don't know where to put the additional space :-) and we just
make the layout uglier.
...
ignore it (as the case of the proportioned manager, which always
expands all the contained views).  This value is the default for all
views which don't have the flag set to expand.
...
\begin{itemize}
\item \texttt{- (GSAutoLayoutAlignment) autolayoutDefaultHorizontalAlignment}, which
should return the default alignment in the horizontal direction for that view
(given the view's class and attributes).
\item \texttt{- (GSAutoLayoutAlignment) autolayoutDefaultVerticalAlignment}, which
...
The container will call \texttt{-autolayoutDefaultHorizontalBorder} of
...
The default implementation of those methods in \texttt{NSView} returns
\texttt{4} for both of them.  Containers and spaces have implementation
