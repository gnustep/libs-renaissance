...
which is the internal name to give to the object.  For example,
\begin{verbatim}
<window id="board"/>
\end{verbatim}
...
\item some other existing object in the application which is made available 
     ...
     name table dictionary (see description of the NSBundle GSMarkup API),
     and then the file can make any connections to those objects by
     referring to them by id, and using the special syntax with the
     hash.
\end{enumerate}
...
FIXME/TODO: Perhaps we could we allow the syntax
\begin{verbatim}
<window delegate="#NSOwner.windowDelegate"/>
\end{verbatim}
to mean setting as delegate the result of calling \texttt{[\#NSOwner
valueForKey: @"windowDelegate"]} ... please note that this can't be
done using normal NIB outlets!  We need a special NIB outlet class
which can perform key-value coding not only on the source (in
writing), but on the target (in reading) as well!  I wouldn't add it
because it complicates things, and confuses non-key-value-coding
gurus.  You can do the same things without it, just in a different
way.  For power users which still want to use it, we could add a
special connector tag which performs this kind of advanced connection,
so power users could still use it by using this advanced connector in
...
\item (FIXME - possibly just Nicola's weird idea - not yet implemented btw)
Some predefined objects have a predefined \texttt{id} given to them
...
the mappings from ids to objects).
...
\item The list of possible meaningful object attributes for the tag.  These are 
converted into outlet connectors at run-time.  The list lists standard
attributes which take a value which is another object (identified by
id).  Because of the way object attributes work (by setting attributes
at run-time using key-value coding), there might be more attributes than
...
The best way to get it right without too much effort is by mimicking
what working examples are doing (such as the CurrencyConverter in
...
connector.  If omitted, \texttt{nil} will be used as target when
