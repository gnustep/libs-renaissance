...
example, all tags representing view and window objects support a
\texttt{class} attribute -- if this attribute is set to a value, 
...
\begin{verbatim}
<button title="Example" class="MyButton" />
\end{verbatim}
...
\subsubsection{The class attribute}
...
\begin{verbatim}
<window class="NPWindow" width="100" height="100" resizable="no" />
\end{verbatim}
...
In detail, when you specify a \texttt{class="XXX"} attribute for a
...
if no \texttt{class} attribute had been specified).  If it is, then
...
the \texttt{class} attribute, and tags which ignore it.
...
specified with the \texttt{class} attribute).  This is mostly used to
...
off using a view or window tag with a \texttt{class} attribute,
...
%\end{itemize}
%
% The \texttt{<xxx>} tag supports the \texttt{class} attribute.
%
...
The \texttt{<image>} tag supports the \texttt{class} attribute.
...
The \texttt{<menu>} tag does not support the \texttt{class} attribute.
...
The \texttt{<menuItem>} tag does not support the \texttt{class} attribute.
...
\texttt{<menuSeparator>} tag does not support the \texttt{class}
attribute.
...
The \texttt{<panel>} tag supports the \texttt{class} attribute.
...
The \texttt{<window>} tag supports the \texttt{class} attribute.
